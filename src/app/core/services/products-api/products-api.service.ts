...
export class ProductsApiService implements IProductsApiService {
  ...
  getAllProductsToUser(user: IUserModel): Promise<any> {
    return this.httpClient.post(`${UrlConstans.apiUrl}9862c578-93a5-4e97-bbbf-966d286ff1dc`,
    user,
    {observe: 'response'}
  ).pipe(
      map((response) => {
  ...
  getAllTransactionToAccount(accountId: string): Promise<any> {
    ...
    return this.httpClient.post(`${UrlConstans.apiUrl}97281e7a-59b1-4c99-9cce-3626b61106d3`,
    accountId,
    {observe: 'response'}
  ).pipe(
      map((response) => {
